...
void LobotServoController::moveServos(LobotServo servos[], uint8_t Num, uint16_t Time)
{
	...
	for (uint8_t i = 0; i < Num; i++) { //循环填充舵机ID和对应目标位置
		...
		//suter change alt
		 buf[index++] = GET_LOW_BYTE(map(servos[i].Position,0,180,500,2500)); //填充目标位置低八位
		 buf[index++] = GET_HIGH_BYTE(map(servos[i].Position,0,180,500,2500));//填充目标位置高八位
...
/**************************************************
 * Function:  getNumber
 * Description： 得到从串口监视器中输入的数据
 * Parameters:   无输入参数
 * Return:       返回输入的角度（int 0-180）
 * Others:
****************************************************/
int LobotServoController::getNumber() {
  String comdata = "";
  String numData = "";
  Serial.println("ready to get position!");
  while (Serial.available() == 0);//空循环，等待串口监视器输入
  ...
  if (comdata.length() > 0)
  {
    ...
    {
      if (comdata[i] >= '0' && comdata[i] <= '9')//只读取数字
        numData += (char)comdata[i];
    }
    return numData.toInt();
  }
}
